...
* vzorec pro výpočet absolutního rozdílu
* vzorec pro výpočet relativního rozdílu
$$d_{ABS}=h_{n}-h_{n-1}$$
...
* nelze rict obecne rict, od jakych hodnot dobre/spatne...? , obecne cim vyssi tim lepsi, lze se odrazit od prumeru pro dany obor - dobra vypovidajici hodnota
...
Vzhledem k tomu, že se způsoby výpočtu některých ukazatelů v různých literaturách liší, bylo by vhodné uživateli zobrazit způsob výpočtu, který je použit v tomto nástroji. Při současné implementaci by se dal ke každé metodě provádějící výpočet přidat také popis výpočtu, například ve formě řetězce. Tyto dvě věci? by však nebyly propojeny na programové úrovni? a vývojář by tak musel při každé změně algoritmu pro výpočet adekvátně měnit popis. Vzhledem k tomu, že se jedná o víceméně jednoduché matematické rovnice, nabízí se mnohem lepší řešení -- uložit rovnice pro výpočet do odpovídajícího abstraktního syntaktického stromu (AST). Tato struktura by umožnila obojí -- vypočítat výsledek i vygenerovat popis výpočtu a jeho složek na základě jednotného zdroje.

Nástroj pro jednoduchost podporuje pouze meziroční analýzu. Lze však vyvodit, že se v praxi může provádět analýza obecně různých časových obodbí, například pro sezónní podniky?. Ke zkvalitnění by se dalo použít i dat z analýzy podnikové sféry prováděné MPO, která uvádí statistiky kvartálně (momentálně jsou využity pouze roční? statistiky).
...
+ obsahuje nektere dalsi pomerove ukazatele, pripadne data potrebna pro jejich vypocet - dalo by se taky vyuzit

Jelikož jsou však tato data uložena jako ?velmi rozsáhlé tabulky programu Excel?, ruční výpočet a zapisování do zdrojového kódu by bylo velmi zdlouhavé. Nabízí se proto automatizované zpracování či umožnění vyplnění těchto hodnot samotným uživatelem v případě jeho zájmu.




* vice roku - nemoznost zobrazit vse - vymyslet, jak by se to dalo zobrazovat
* výpočet položek přes účty - detailnější, možnost \"cast polozky\" - otazka je, zda existuje nejaky standardizovany format
...
* upozorneni na evidentni veci - hospodareni se ztratou apod
* sirsi zaber mezipodnikoveho srovnani - nejen to co je

* pro usnadneni ovladani: offline?, cashflow - preddefinovane polozky, pomerove uk: moznost tvorby vlastnich/editace + moznost zadat primo cleny vypoctu (ne cele vykazy)
* samozrejmost - dukladnejsi osetreni uzivatelskych vstupu + rozsireni importu - zkracene vykazy / ucelove cleneni vzz apod
...
* navrh - srovnani softwaru - po diskuzi s lidmi z praxe je velmi casto pouzivan pouze excel
